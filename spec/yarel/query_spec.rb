...
describe Yarel::Query do
  ...
  context "#table" do
    it "should accept a table name" do
      Yarel::Query.new(connection, "the.table").table.should == "the.table"
    end
  end

  context "#from" do
    it "should be chainable" do
      Yarel::Query.new(connection, nil).from("answers.new_table").tap do |query|
    ...
    it "should be incorporated into the generated YQL" do
      Yarel::Query.new(connection, "old.table").from("new.table").to_yql.should == "SELECT * FROM new.table"
    end
    
    it "should incorporate successive calls" do
      Yarel::Query.new(connection, "old.table").from("new.table").from("newest.table").to_yql.should == "SELECT * FROM newest.table"
    end
  end
  
  context "#projections" do
    it "should be * by default" do
      Yarel::Query.new(connection, "some.table").projections.should == ["*"]
    end
    
    it "should be initializable through the constructor" do
      Yarel::Query.new(connection, "some.table", :projections => ["foo", "bar"]).projections.should == ["foo", "bar"]
    end
  end
  
  context "#project" do
    it "should be chainable" do
      Yarel::Query.new(connection, "some.table").project("foo", "bar").tap do |query|
        query.should be_kind_of(Yarel::Query)
    ...
    it "should convert the given arguments to strings" do
      Yarel::Query.new(connection, "some.table").project(:foo).projections.should == ["foo"]
    end
    
    it "should be incorporated into the generated YQL for a single column" do
      Yarel::Query.new(connection, "some.table").tap do |query|
  ...
  context "#result_limit (reader)" do
    it "should be nil by default" do
      Yarel::Query.new(connection, "some.table").result_limit.should be_nil
    end
    
    it "should be initializable through the constructor" do
      Yarel::Query.new(connection, "some.table", :result_limit => 10).result_limit.should == 10
    end
  end
  
  context "#limit (writer)" do
    it "should be chainable" do
      Yarel::Query.new(connection, "some.table").limit(10).tap do |query|
        query.should be_kind_of(Yarel::Query)
    ...
    it "should be incorporated into the generated YQL" do
      Yarel::Query.new(connection, "some.table").limit(10).to_yql.should == "SELECT * FROM some.table LIMIT 10"
    end
    
    it "should take an optional second argument for specifying the offset" do
      Yarel::Query.new(connection, "some.table").limit(5, 15).tap do |query|
    ...
    it "should incorporate offset into the generated YQL" do
      Yarel::Query.new(connection, "some.table").limit(5, 15).to_yql.should == "SELECT * FROM some.table LIMIT 5 OFFSET 15"
    end
  end
  
  context "#conditions" do
    it "should be empty by default" do
      Yarel::Query.new(connection, "some.table").conditions.should be_empty
    end
    
    it "should be initializable through the constructor" do
      Yarel::Query.new(connection, "some.table", :conditions => ["this_column = '5'"]).conditions.should == ["this_column = '5'"]
    end
  end
  
  context "#where" do
    it "should accept a hash of a single parameter" do
      Yarel::Query.new(connection, "some.table").where(:this_column => 5).conditions.should == ["this_column = '5'"]
    end
    
    it "should accept a hash of multiple parameters" do
      Yarel::Query.new(connection, "some.table").
    ...
    it "should handle subqueries" do
      Yarel::Query.new(connection, "some.table").where(:this_column => Yarel::Query.new(connection, "sub_table").project("sub_table_column")).to_yql.should ==
  ...
  context "#sort" do
    it "should add the given sort field to the query" do
      Yarel::Query.new(connection, "some.table").sort("Rating.AverageRating").sort_field.should == "Rating.AverageRating"
    end
  ...
  #     Yarel::Query.new(connection, "some.table").should_not be_executed
  #   end
  # end
  
  context "#all", :pending => true do
